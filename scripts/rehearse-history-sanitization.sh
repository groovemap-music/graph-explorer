...
git -C "${backup_repo}" bundle create "${output_root}/graph-explorer-pre-rewrite.bundle" --all
git bundle verify "${output_root}/graph-explorer-pre-rewrite.bundle" > "${output_root}/bundle-verify.txt" 2>&1
...
shasum -a 256 \
  "${output_root}/graph-explorer-pre-rewrite.bundle" \
